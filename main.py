    ...
    try:
        ...
        if chat_request.files:
            for i, file_ref in enumerate(chat_request.files):
                logger.debug(
                    f"📄 File {i + 1}: ID={file_ref.id}, Type={file_ref.type}, Name={file_ref.name}"
                )
        ...
        if stream:
            ...
            async def streaming_generator():
                try:
                    async for chunk in router.route_and_process_streaming(
                        chat_request, auth_header
                    ):
                        yield chunk
...
async def debug_conversations(authorization: str = Header(None)):
    ...
    conversations = []
    for conv_id in list(conversation_state_manager.states.keys())[
        :10
